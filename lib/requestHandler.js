    ...
    co(function *() {
      ...
      .then(() => {
        return new Promise((resolve, reject) => {
          ...
          cltSocket.on('data', (chunk) => {
            requestStream.push(chunk); // 存储代理服务器接收到的请求trunk
            if (!resolved) {
              resolved = true;// 为了取第一帧数据
              try {
      ...
      // determine the request target
        ...
        // server info from the original request
          ...
          // 若最先创建的代理服务器是http协议的, 那么当接收到https协议目标网站的请求时, 到这里每次都会重新创建一个新的真正的发往目标服务器的https协议的代理服务器
      ...
      .then((serverInfo) => {
        ...
        return new Promise((resolve, reject) => {
          const conn = net.connect(serverInfo.port, serverInfo.host, () => { // 连接到创建的https server, 该https server才是真正的代理服务器
          //throttle for direct-foward https
            // 把http协议代理服务器和net服务器pipe起来, 而net服务器连接真正的https代理服务器, 这样所有发往http代理服务器的请求都会转发到真正的https代理服务器, 而真正的https代理服务器收到的响应又会传回http协议代理服务器, 从而实现数据流通
...
class RequestHandler {
  ...
  constructor(config, rule, recorder) {
    ...
    reqHandlerCtx.httpsServerMgr = new HttpsServerMgr({
      ...
      hostname: '127.0.0.1', // 这里的hostname毫无用处
    });
